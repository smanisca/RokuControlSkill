...
namespace RokuDeviceLib
{
    public class RokuClient
    {
        ...
        {
            var client = new HttpClient();
            var requestUrl = String.Format("{0}query/apps", endpoint);
            var result = await client.GetStreamAsync(requestUrl);
        ...
        public static async void PressKey(String endpoint, String key)
        {
            var client = new HttpClient();
            var requestUrl = String.Format("{0}keypress/{1}", endpoint, key);
            var result = await client.PostAsync(requestUrl, null);
        }
        public static async void LaunchAppByName(String endpoint, String appname)
        {
            IEnumerable<RokuApp> apps = await RokuClient.ListDeviceApps(endpoint);
            foreach (var app in apps)
            {
                if (String.Equals(app.Name, appname, StringComparison.OrdinalIgnoreCase))
                {
                    await RokuClient.LaunchApp(endpoint, app.Id);
                    break;
                }
            }           
        }
